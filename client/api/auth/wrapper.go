...
func Wrapper(r resolver.Resolver) server.Wrapper {
...
func (a authWrapper) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if len(ns) == 0 {
		req.Header.Set(namespace.NamespaceKey, endpoint.Network)
	}
	...
	res := &auth.Resource{Type: "service", Name: endpoint.Name, Endpoint: resEndpoint}
